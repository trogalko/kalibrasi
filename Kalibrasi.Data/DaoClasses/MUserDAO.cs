...
namespace Kalibrasi.Data.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the MUser Entity. It will perform database oriented actions for
	/// a entity of type 'MUserEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class MUserDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public MUserDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "MUserEntity", new MUserEntityFactory())
		{
		}
		...
		internal MUserDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}
		...
		/// <summary>
		/// Deletes from the persistent storage all 'MUser' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="mhakAksesInstance">MHakAksesEntity instance to use as a filter for the MUserEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity mhakAksesInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(Kalibrasi.Data.EntityType.MUserEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(mhakAksesInstance, fields);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="mhakAksesInstance">MHakAksesEntity instance to use as a filter for the MUserEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntity entityWithNewValues, ITransaction containingTransaction, IEntity mhakAksesInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(Kalibrasi.Data.EntityType.MUserEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(mhakAksesInstance, fields);
